...
-- local in_tardis = (LocalPlayer():GetTardisData("interior") == nil) and not (LocalPlayer():GetTardisData("thirdperson"))
...
local function env_corotinefunction()
	...
	-- is inside
		...
		if LocalPlayer():GetTardisData("interior") ~= nil then
			if IsValid(LocalPlayer():GetTardisData("exterior")) then  -- for whatever reason, tardis data may not be wiped when exiting, leading the entity to be nil
				is_inside = not LocalPlayer():GetTardisData("thirdperson")
			end
		end
	-- ZDis
	...
	if is_inside and not in_water then
		...
		-- Roof pos
			...
			if (LocalPlayer():GetTardisData("interior") ~= nil) and not (LocalPlayer():GetTardisData("thirdperson")) then
				roof_pos = LocalPlayer():GetTardisData("interior"):GetPos() + Vector(0, 0, 0)
				roof_type = 5

					if LocalPlayer():GetTardisData("exterior"):Locked() == true then
						roof_type = 0
					end
			end
