	...
	function StormFox2.DownFall.CalculateDrop( nDis, nSize, nTries, vNorm, ignoreVel, nMaxDistance, tTemplate )
		...
		if LocalPlayer():GetTardisData("interior") ~= nil then
			if IsValid(LocalPlayer():GetTardisData("exterior")) then  -- for whatever reason, tardis data may not be wiped when exiting, leading the entity to be nil
				view.pos = LocalPlayer():GetTardisData("exterior"):GetPos()
			end
		end
...
if CLIENT then
	...
	function pt_meta:CreateParticle( vEndPos, vNorm, hitType, hitNorm, maxDistance )
		...
		if LocalPlayer():GetTardisData("interior") ~= nil then
			if IsValid(LocalPlayer():GetTardisData("exterior")) then  -- for whatever reason, tardis data may not be wiped when exiting, leading the entity to be nil
				view = LocalPlayer():GetTardisData("exterior"):GetPos()
			end
		end
	...
	local function ParticleRender()
		...
		if LocalPlayer():GetTardisData("interior") ~= nil then
			if IsValid(LocalPlayer():GetTardisData("exterior")) then  -- for whatever reason, tardis data may not be wiped when exiting, leading the entity to be nil
				v = LocalPlayer():GetTardisData("exterior"):GetPos()
			end
		end
	...
	function StormFox2.DownFall.AddTemplateSimple( tTemplate, vEndPos, hitType, hitNorm, nDistance, vNorm, maxDistance )
		...
		if not nDistance then
			...
			if LocalPlayer():GetTardisData("interior") ~= nil then
				if IsValid(LocalPlayer():GetTardisData("exterior")) then  -- for whatever reason, tardis data may not be wiped when exiting, leading the entity to be nil
					p = LocalPlayer():GetTardisData("exterior"):GetPos()
				end
			end
